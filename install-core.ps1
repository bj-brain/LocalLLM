...
foreach ($m in $SelectedModels) {
    ...
    # Cleanup old Dolphin 2.9 versions if upgrading to Dolphin 3.0
    if ($m.Local -eq "dolphin-local" -and $m.File -match "Dolphin3.0") {
        $legacyFiles = @(
            "$USB_Drive\models\dolphin-2.9-llama3-8b-Q4_K_M.gguf",
            "$USB_Drive\models\dolphin-2.9-llama3-8b-Q5_K_M.gguf"
        )
        foreach ($oldFile in $legacyFiles) {
            if (Test-Path $oldFile) {
                Write-Host "      Found old Dolphin 2.9 file. Deleting to free up space..." -ForegroundColor Yellow
